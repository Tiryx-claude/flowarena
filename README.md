...
Schwierigkeit und Thema, droppen Bars über 3 feste Strophen, teilen
Recordings als Posts und werden bewertet.
...
14. ✅ Reimwort-System, generalüberholt — >24.000 zusätzliche echte Wörter pro Sprachraum (DE/EN/RU) aus einer Skript-Pipeline, Anti-Wiederholung (merkt sich benutzte Wörter, erst ab 85% "verbraucht" wird zurückgesetzt), von Hand geprüfte generative Komposita-Ergänzung (Deutsch), optionaler Street-Modus (härtere Battle-Rap-Reimwörter/-Themen), neues Themenfeld "Humor" (siehe docs/GAMEPLAY.md Abschnitt 7.1)
15. ✅ Reimqualität, grundlegend überarbeitet — Stamm-Vielfalt gegen künstliche/fast identische Wortreihen (z.B. "mitgemacht/rumgemacht/angemacht" als eine Wurzel statt fünf verschiedene Wörter erkannt), Familien-Auswahl gewichtet gegen wurzelarme Reimfamilien, längere Anti-Wiederholung, performant auch bei sehr großen Wortfamilien (siehe docs/GAMEPLAY.md Abschnitt 7.1, Punkt 4b)
16. ✅ Echte Reimschemata & Punchline-Wortschatz — Strophen nutzen zufällige Buchstaben-Schemata mit max. 2 Wiederholungen je Buchstabe (ABCCB, ABCAB, ABCBA, AABCB, ABACB u.a. — NIE 3+ Zeilen auf demselben Klang), jeder Buchstabe eine eigene, garantiert andere Reim-Familie, das erzwingt rechnerisch mindestens 3 verschiedene Familien pro 5-Zeilen-Strophe; von Hand geprüfte Battle-Rap-/Jugend-/Streamer-Wortschicht (assets/js/rhyme-slang.js, DE 29/EN 19/RU 11 Familien: Ego, Flex, Pleite, Skill, Knast, fake, snake, ban, clan, жир, понты, бан u.v.m.) ist der bevorzugte Standard-Pool in beiden Modi; explizite Sperrliste für bekannte "Wörterbuch-Reim"-Familien (aktuell: das deutsche "-ier", z.B. Revier/Papier/Quartier/Klavier — formelle Lehnwörter, die trotz zufälliger Themen-Tags nicht wie gesprochene Rap-Sprache klingen); Zwei-Pass-Familien-Merge behebt wortübergreifende Duplikate zwischen Kernbank/Zusatzbank/rhyme-slang.js (vorher konnten zwei verschiedene Familien-IDs mit demselben Klang dasselbe Wort liefern, z.B. "shame" zweimal in einer Strophe); Street-Modus verstärkt gezielt "scharfe" Battle-Energie, Normal-Modus "weiche" Status-/Humor-Energie; persistente Familien-Historie über Sessions/Spiele hinweg sorgt dafür, dass sich auch nach mehreren Runden noch frisch anfühlt (siehe docs/GAMEPLAY.md Abschnitt 7.1, Punkte 4c–4f/6b/6c)
17. ✅ Startscreen als Game statt Tutorial-Seite — Konfiguration (Schwierigkeit, Beat, Thema, Street-/Streamer-Modus) steht direkt als interaktive Cards auf der Startseite statt hinter einem Chip-Klick versteckt, kürzerer Hero-Text, Gameplay-Erklärung eingeklappt hinter "Wie genau?", "Challenge starten" als mit Abstand dominantester Button; Strophenanzahl ist keine Einstellung mehr — jede Challenge läuft fest über 3 Strophen à 5 Zeilen (`GAMEPLAY_CONFIG.stanzasPerChallenge`), Premium bietet dafür weiterhin unbegrenzt Challenges/Tag (siehe docs/GAMEPLAY.md Abschnitt 3c, docs/SHOP.md)
18. ✅ Explizite Reimpaar-Qualitätsprüfung + eindeutiges HUD — nach der Wortauswahl prüft `hasWeakPairing()` explizit, ob ein Wortpaar desselben Buchstabens trotz unterschiedlicher Schreibweise stammverwandt ist (z.B. "bezahlen"/"heimzahlen"); bei einer schwachen Paarung probiert `pickFamilyAndWords()` bis zu 4 andere Reimfamilien, bevor die Strophe (nie unvollständig) ausgegeben wird; "Als Nächstes"-Vorschau zeigt keine Zeilennummern mehr, die mit den 1-4-Feldnummern des aktuellen Word-Racks verwechselt werden konnten, sondern eine eigene Überschrift (siehe docs/GAMEPLAY.md Abschnitt 7.1, Punkt 4g)
